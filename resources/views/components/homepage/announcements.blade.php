...
<div id="pengumuman" class="relative flex justify-center items-center mt-20 mb-20">
    ...
    <div class="w-[85%] max-w-7xl space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse($announcements as $announcement)
                    <div
                        class="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 hover:-translate-y-2">

                        <!-- Image with Overlay -->
                        <div class="relative w-full aspect-[4/3] overflow-hidden bg-gradient-to-br from-purple-100 to-blue-100">
                            ...
                            @else
                                <img src="{{ asset('images/default-announcement.png') }}" alt="Default announcement"
                                    class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                            @endif
                            ...
                            <div class="absolute top-4 left-4">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold backdrop-blur-sm shadow-lg
                    ...
                    ($announcement->category === 'Event' ? 'bg-green-500 text-white' :
                        ($announcement->category === 'Teknis' ? 'bg-red-500 text-white' : 'bg-purple-500 text-white'))) }}">
                            ...
                            <div class="absolute bottom-4 left-4">
                                <div
                                    class="flex items-center gap-2 text-white backdrop-blur-sm bg-black/30 px-3 py-1 rounded-lg">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span
                                        class="text-sm font-semibold">{{ \Carbon\Carbon::parse($announcement->date)->format('d M Y') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-4 sm:p-5">
                            ...
                            <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                                ...
                                <!-- Read More Button -->
                                <button x-data
                                    @click="$dispatch('open-announcement-modal', { announcement: {{ json_encode($announcement) }} })"
                                    class="mt-4 w-full sm:w-auto py-2 px-4 inline-flex items-center text-purple-600 hover:text-purple-700 font-semibold text-sm group/btn">
                                    <span>Baca</span>
    ...
    <div x-data="{ 
    ...
    }" @open-announcement-modal.window="announcement = $event.detail.announcement; open = true" x-show="open" x-cloak
        class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">

        <!-- Backdrop -->
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="open = false" x-show="open"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-full sm:max-w-xl md:max-w-2xl w-full overflow-hidden" x-show="open"
                ...
                <div class="relative h-64 bg-gradient-to-br from-purple-100 to-blue-100">
                    ...
                    <div class="absolute bottom-4 left-6">
                        <span
                            class="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-bold backdrop-blur-sm shadow-lg bg-white/90 text-purple-600"
...
">
                    ...
                    <div class="flex items-center gap-4 text-sm text-gray-500 mb-6 pb-6 border-b border-gray-200">
                        <div class="flex items-center gap-2">
                            ...
                            <span
                                x-text="announcement?.date ? new Date(announcement.date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : ''"></span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
