...
class GallerySeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Downloading images untuk Galleries...');

        foreach ($galleries as $galleryData) {
            // Download gambar dan dapatkan path lokal
            $imageUrl = $galleryData['image_url'];
            $imagePath = $this->downloadImage($imageUrl, 'galleries');

            // Skip jika download gagal
            if (!$imagePath) {
                $this->command->warn("Skip gallery: {$galleryData['title']} (gagal download gambar)");
                continue;
            }

            // Ganti image_url dengan image path lokal
            unset($galleryData['image_url']);
        ...
        $this->command->info('Seeding Galleries selesai!');
    }
}
